...
const getHero = async (req, res) => {
    try {
        ...
        const { _id: uid } = req.user;
        console.log(id);
        const checkHeroExist = await HeroModel.findOne({ _id: id, uid }).populate('tags').lean();
...
const editHero = async (req, res) => {
    try {
        ...
        const hero = await HeroModel.findOneAndUpdate({ _id: body._id, uid }, body, { new: true })
...
const editTag = async (req, res) => {
    try {
        const { id } = req.params
        const { name } = req.body;
        const result = await TagModel.findByIdAndUpdate(id, { name: name }, { new: true })
...
module.exports = { getHeroes, addHero, deleteHero, editHero, getHero, getTags, addTag, editTag, deleteTag }